...
_VS_CACHE: Dict[str, Tuple[FAISS, str]] = {}   # {index_dir: (vs, hash)}
_VS_LOCKS: Dict[str, Lock] = {}
...
def background_index_builder():
    """
    Consumes REINDEX_QUEUE and (re)builds the right audience index.
    Recomputes access flags from DB on every job (authoritative).
    """
    while True:
        try:
            job = redis_client.blpop(REINDEX_QUEUE, timeout=5)
            if not job:
                continue
            _, payload = job
            args = json.loads(payload.decode("utf-8")) if isinstance(payload, (bytes, bytearray)) else json.loads(payload)
            uid = int(args["user_id"])

            with pg_conn() as c:
                role_names, features = fetch_user_roles_features(c, uid)
                superadmin = is_superadmin(c, uid)
                ...
                if superadmin:
                    ...
                    all_projects = db_has_feature(c, uid, FeatureID.ALL_PROJECTS)
                    budget_ok    = db_has_feature(c, uid, FeatureID.BUDGET_INFO)
                    user_mgmt_ok = db_has_feature(c, uid, FeatureID.USER_MANAGEMENT)
                    notes_ok     = db_has_feature(c, uid, FeatureID.NOTES)

                if all_projects:
                    _build_admin_index(
                        c,
                        include_budget=budget_ok,
                        include_users=user_mgmt_ok,   # include user directory only if allowed
                        viewer_user_id=uid,
                        superadmin=superadmin
                    )
                else:
                    _build_manager_index(
                        c,
                        uid,
                        include_budget=budget_ok,
                        include_users=user_mgmt_ok    # managers can optionally get users if feature allows
                    )
        except Exception as e:
            logger.error(f"[IndexBuilder] {e}")
...
def _build_index(index_dir: str, docs: List[Document], max_chunk_tokens: int = 20000):
    ...
    idx_hash = hashlib.sha256(("|".join(d.metadata.get("chunk_id","") for d in chunks)).encode("utf-8")).hexdigest()
    with open(os.path.join(index_dir, "index.hash"), "w", encoding="utf-8") as f:
        f.write(idx_hash)
    # bust in-memory cache for this dir
    _VS_CACHE.pop(index_dir, None)
...
# simple Redis-backed debounce + enqueue
REINDEX_QUEUE = "reindex_jobs"

def request_index(audience_key: str, build_args: Dict[str,Any], *, force: bool=False, debounce_sec: int=60) -> None:
    # debounce for 60s to avoid storms
    key = f"reindex:debounce:{audience_key}"
    if force or not redis_client.get(key):
        redis_client.rpush(REINDEX_QUEUE, json.dumps(build_args))
        redis_client.set(key, "1", ex=60)

def _audience_for(user_id: int, all_projects: bool) -> str:
    return f"admin_for_{user_id}" if all_projects else f"manager_{user_id}"



def ensure_index_fast(conn, user_id: int) -> Tuple[str, bool, bool]:
    """
    Cheap, feature-aware ensure:
      - computes current 'content + access' fingerprint,
      - enqueues background rebuild if changed/missing,
      - returns (index_dir, all_projects, budget_ok) immediately.

    Never walks projects or rebuilds on the request thread.
    """
    # ---- 1) Access flags (same semantics as your existing code) ----------------
    ...
    idx_dir = _aud_admin_dir(user_id) if all_projects else _aud_manager_dir(user_id)
    aud_key = _audience_for(user_id, all_projects)

    # ---- 2) Lightweight change signals (O(1) queries) --------------------------
    # We rely on MAX(updated_at) (or MAX(created_at) if updated_at is null) as a coarse
    # invalidation signal for projects, notes, and (optionally) users.
    with conn.cursor() as cur:
        cur.execute("SELECT COALESCE(MAX(updated_at), MAX(created_at)) FROM project_management_project")
        p_ts = str(cur.fetchone()[0])

        n_ts = ""
        if notes_ok:
            try:
                cur.execute("SELECT COALESCE(MAX(updated_at), MAX(created_at)) FROM project_management_projectnotes")
                n_ts_val = cur.fetchone()[0]
                n_ts = str(n_ts_val)
            except Exception:
                n_ts = ""  # table might not exist; ignore safely

        u_ts = ""
        if user_mgmt_ok:
            try:
                cur.execute("SELECT COALESCE(MAX(updated_at), MAX(created_at)) FROM user_management_user")
                u_ts = str(cur.fetchone()[0])
            except Exception:
                u_ts = ""

    # ---- 3) Feature fingerprint (roles + features list) ------------------------
    feat_payload = {
        "roles": sorted([r.lower() for r in role_names]),
        "features": sorted(list(features)),  # keep your DB-derived feature names
    }
    feat_fp = hashlib.sha256(
        json.dumps(feat_payload, sort_keys=True, ensure_ascii=False).encode("utf-8")
    ).hexdigest()

    # ---- 4) Composite fingerprint (content + access + format rev) --------------
    fp_now_raw = "|".join([
        p_ts or "",
        n_ts or "",
        u_ts or "",
    ...
    fp_now = hashlib.sha256(fp_now_raw.encode("utf-8")).hexdigest()

    # ---- 5) Compare & enqueue if missing or changed ----------------------------
    fp_key = f"aud_fp:{aud_key}"
    fp_prev = redis_client.get(fp_key)
    idx_missing = not os.path.exists(idx_dir)

    if idx_missing or not fp_prev or fp_prev.decode() != fp_now:
        # minimal build args; builder recomputes flags to avoid staleness
        build_args = {"user_id": user_id, "aud_key": aud_key}
        request_index(aud_key, build_args, force=idx_missing)
        # remember the new fp for the next calls; TTL avoids unbounded persistence
        redis_client.set(fp_key, fp_now, ex=3600)

    # ---- 6) Serve immediately (last known good index) --------------------------
    return idx_dir, all_projects, budget_ok


# =---------------------------------------Helper functions ------------------------------

def _read_index_hash(index_dir: str) -> Optional[str]:
    p = os.path.join(index_dir, 'index.hash')
    return open(p, 'r', encoding='utf-8').read().strip() if os.path.exists(p) else None

def get_vs_cached(index_dir: str) -> Optional[FAISS]:
    if index_dir not in _VS_LOCKS:
        _VS_LOCKS[index_dir] = Lock()
    with _VS_LOCKS[index_dir]:
        h = _read_index_hash(index_dir)
        cached = _VS_CACHE.get(index_dir)
...
# Start background threads AFTER all helpers/constants are defined
threading.Thread(target=background_rebuilder, daemon=True).start()          # your tabular updater
threading.Thread(target=background_index_builder, daemon=True).start()      # new index builder

# Optional: move policy updater out of request path
def policy_rebuilder_daemon():
    while True:
        try:
            maybe_update_policy_index()
        except Exception as e:
            logger.error(f"[PolicyRebuilder] {e}")
        time.sleep(3600)

threading.Thread(target=policy_rebuilder_daemon, daemon=True).start()
